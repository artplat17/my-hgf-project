		...
		switch ($mode)
		{
			...
			case 'profile_info':
				...
				*/
				...
				if ($config['allow_birthdays'])
				{
					$s_birthday_day_options = '<option value="1"' . ((!$data['bday_day']) ? ' selected="selected"' : '') . '>1</option>';
					for ($i = 2; $i < 32; $i++)
					{
					...
					$s_birthday_month_options = '<option value="1"' . ((!$data['bday_month']) ? ' selected="selected"' : '') . '>1</option>';
					for ($i = 2; $i < 13; $i++)
					{
					...
					$now = getdate();
					$s_birthday_year_options = '<option value="1922"' . ((!$data['bday_year']) ? ' selected="selected"' : '') . '>1922</option>';
					for ($i = $now['year'] - 99; $i <= $now['year'] - 18; $i++)
					{
