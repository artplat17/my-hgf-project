	...
	*/
	...
	if ($mode == 'post' && $post_data['post_id'] == 0) {
        if ($user->data['is_registered'] && !$user->data['is_bot']) {
            $title = utf8_normalize_nfc(request_var('subject', '', true));
            $words = preg_split('/\s+/', $title);
            if (count($words) < 3 || utf8_strlen($title) < 10) {
                $error_msg = '<strong class="limit-msg.title-limit">Sorry, your topic title must consist of at least three words and be at least 10 characters long. Please provide a more descriptive title.</strong>';
    ...
    // Check if user is not an admin and check if user has exceeded daily reply limit
    if ($auth->acl_get('a_')) {
    ...
    if ($row['reply_count'] >= $reply_limit) {
        ...
        if ($time_left < 60) {
        $remaining_time = $time_left . " second(s)";
        } elseif ($time_left < 3600) {
        $remaining_time = floor($time_left / 60) . " minute(s) and " . ($time_left % 60) . " second(s)";
        } else {
        $remaining_time = floor($time_left / 3600) . " hour(s) and " . floor(($time_left % 3600) / 60) . " minute(s)";
    }
        trigger_error("<strong class='limit-msg.post-limit'>You have reached your daily response limit. Please try to submit your reply again in $remaining_time ;)</strong>");
    }
}
	...
	$data_ary['topic_title'] = truncate_string($data_ary['topic_title'], 90);
