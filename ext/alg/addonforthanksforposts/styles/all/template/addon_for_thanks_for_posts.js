...
//        console.log(btnsLike);
    ...
    function togle_thanks(data) {
        ...
        if (data.THANKS && data.THANKS_POSTLIST_VIEW) {
            var updDiv = "<div class='notice hotlist'>";
            if (!data.S_POST_ANONYMOUS && !data.S_IS_BOT && data.S_MOD_THANKS) {
                updDiv = updDiv + "<ul class='post-buttons' style='float: right;position: static;margin-top: 3px;'>";
                updDiv = updDiv + "<li>";
            ...
            if (!data.S_POST_ANONYMOUS && !data.S_IS_BOT) {
                updDiv = updDiv + data.THANK_TEXT + data.POST_AUTHOR_FULL + data.THANK_TEXT_2;
                updDiv = updDiv + "<br>";
                updDiv = updDiv + data.THANKS;
            }
            updDiv = updDiv + "</div >";
        ...
        if (!data.S_POST_ANONYMOUS && data.THANKS_COUNTERS_VIEW) {
            $("dd[data-user-receive-id= " + data['POSTER_ID'] + "]").each(function () {
                ...
                if (data.POSTER_RECEIVE_COUNT == 0) {
                    rcv = "<strong>" + LA_RECEIVED + ":&nbsp;" + "0 &#128293;" + "</strong>";
                    $(this).html(rcv);
                } 
		else {
			rcv = "<strong>" + LA_RECEIVED + ":&nbsp;<a href='" + data.POSTER_RECEIVE_COUNT_LINK + "'>" + data.POSTER_RECEIVE_COUNT + "</a>" + "</strong>";
			$(this).html(rcv);
                }
            });
