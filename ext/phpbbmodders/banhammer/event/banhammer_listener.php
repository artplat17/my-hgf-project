...
class banhammer_listener implements EventSubscriberInterface
{
	...
	public function do_ban_hammer_stuff($event)
	{
		...
		if (!$this->request->is_set('bh') || ($this->request->is_set('bh') && $this->request->is_set('confirm_key') && !confirm_box(true)))
		{
			...
			$this->template->assign_vars(array(
				...
				'BH_DEL_TOPICS'		=> $this->config['bh_del_topics'],
		...
		if (!confirm_box(true))
		{
			$hidden_fields = array(
				...
				'del_topics'		=> $this->request->variable('del_topics', 0),
			...
			$message .= ($hidden_fields['del_topics'])		? $this->user->lang['BH_DEL_TOPICS'] . '<br>' : '';
		...
		if ($this->request->variable('del_topics', 0))
		{
			$this->bh_del_topics();
		}
	...
	private function bh_del_topics()
	{
		$user_id = (int) $this->user_id;
		$sql = "SELECT topic_id
				FROM " . TOPICS_TABLE . "
		...
		delete_topics('topic_id', array_keys($topics));
	}
