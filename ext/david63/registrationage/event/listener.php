...
class listener implements EventSubscriberInterface
{
	 ...
	 * @access public
	 */
	public function __construct(config $config, user $user, language $language, template $template, request $request, auth $auth, log $log, functions $functions, ra_functions $ra_functions)
	{
		$this->config       = $config;
		$this->user         = $user;
	...
	public function add_register($event)
	{
		...
		$this->language->add_lang('ucp_registrationage', $this->functions->get_ext_namespace());

		$data = $event['data'];

		$this->template->assign_vars([
			'MINIMUM_AGE' => $this->language->lang('MINIMUM_AGE', $this->config['registration_age']),

			'RA_NAMESPACE' => $this->functions->get_ext_namespace('twig'),

			'REGISTRATION_AGE_DAY' 		=> $this->today->format('d'),
			'REGISTRATION_AGE_MONTH'	=> $this->today->format('m'),
			'REGISTRATION_AGE_SHOW' 	=> $this->config['registration_age_show'],
			'REGISTRATION_AGE_YEAR' 	=> $this->today->format('Y'),
			'YEAR_START' 				=> $this->today->format('Y') - self::CENTURY,
	...
	public function check_register($event)
	{
		...
		else
		{
			...
			if ($user_age < $this->config['registration_age'])
			{
				...
				{
					$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'REGISTRATION_AGE_LOG_FAIL', time(), [$data['username'], $user_age]);
				}
	...
	public function profile_view($event)
	{
		...
		$this->template->assign_vars([
			'REGISTRATION_AGE' => $this->ra_functions->convert_birthdate($member['user_registration_birthdate'], '|d M Y|'),
